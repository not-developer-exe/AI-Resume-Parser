...
try:
    with open("ml_job_classifier.pkl", "rb") as f:
        job_classifier_model = pickle.load(f)
except FileNotFoundError:
    job_classifier_model = None
...
async def parse_and_save_resume(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    # Save to DB and grab the generated ID
    saved_resume = crud.create_or_update_resume(db=db, resume_data=structured_data)
    structured_data.id = saved_resume.id
...
@app.get("/analyze-resume/{email}")
async def analyze_resume_endpoint(email: str, db: Session = Depends(get_db)):
    try:
        personal_info = db.query(models.PersonalInfo).filter(models.PersonalInfo.email == email).first()
        ...
        if job_classifier_model:
            predicted_domain = job_classifier_model.predict([resume_text])[0]
        
        # Inject it into our response data
        score_data["predicted_domain"] = predicted_domain
        
        existing_score = db.query(models.ResumeScore).filter(models.ResumeScore.resume_id == resume.id).first()
        from datetime import datetime
        if existing_score:
...
async def get_suggestions(email: str, db: Session = Depends(get_db)):
    try:
        ...
        resume_data = {
            ...
            "work_experience": [{"company": w.company, "description": w.description} for w in resume.work_experiences]
        }
        
        prompt = f"""
        Analyze this resume data and provide specific, actionable suggestions for improvement.
        Resume Data: {json.dumps(resume_data, indent=2)}
        Provide suggestions in JSON format with categories: 'content', 'skills', 'formatting', 'impact'.
        Each category should contain an array of objects with a 'text' string field.
        """
